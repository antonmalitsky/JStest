/* 
    ...
    Можно увеличить сжатие, добавив к данным символам дополнительные знаки из ASCII.
    ...
    Код подготовлен для запуска с выводом результатов тестирования.
    Чтобы выводился результат десериализации - раскомментировать 82 строку (написал комментарий в функции makeAndShowResult()). 
*/
...
/*
    ...
    - вместо каждого числа нахожу разность его с предыдущим (но первое число остается как есть),
    ...
    При сериализации обрабатываются только целые числа от 1 до 300 включительно, остальное пропускается,
    поэтому при десериализации в массиве будут только удовлетворяющие данному условию целые числа.
*/
function serialize(arr) {
    ...
    for (let i = 0; i < sorted.length; i++) {
        ...
        if (result.length) {
            diff = sorted[i] - sorted[j]; // разность чисел
        } else {
